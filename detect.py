...
test_path = ROOT_DIR +"/new/processed_dog_2/"
...
def test(test_loader, model):
    ...
    for aug in range(1):
        ...
        for i, (images, filepath) in tqdm(enumerate(test_loader)):
            ...
            # print(cat_prob)
            # print(type(cat_prob))
            # print(type(prob.numpy().tolist()))
            # exit(0)
            csv_map[filepath] = prob.numpy().tolist()
    df = pd.DataFrame(csv_map, index=[0])
    df = df.transpose()

    df.to_excel("prediction_result.xlsx")
...
class TestImageFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = pil_loader(os.path.join(self.root, filename))
...
# cats = os.path.join(split_train_path, 'cats')
# dogs = os.path.join(split_train_path, 'dogs')
# for root, dirs, files in os.walk(os.path.join(full_train_path, 'dogs')):
#     for file in files:
#         if int(file.split('.')[1]) >= 12500:
#             shutil.copyfile(os.path.join(full_train_path, 'dogs', file), os.path.join(dogs, file))
# for root, dirs, files in os.walk(os.path.join(full_train_path, 'cats')):
#     for file in files:
#         if int(file.split('.')[1]) >= 12500:
#             shutil.copyfile(os.path.join(full_train_path, 'cats', file), os.path.join(cats, file))
